...
* 클래스 영역에 존재하는 정보를 기반으로 힙 메모리에 올린 것을 의미    
* 클래스는 객체의 공통적인 스탯을 나타냄 
...
# 자바는 Call By Value?    
자바는  Call By Value                
자바의 참조는 실제 대상이 아닌 참조 대상의 주소값을 복사하는 방식이다.   

    
참조하는 대상이 안바뀌는 것일 뿐이지, 실제 대상의 값은 실제 참조값을 통해서 변경되는 것이다.

Call By Value : -> 대상이 안 바뀔 뿐이지 -> 실제 대상의 값이 변경되는건 어쩔수없

...
`테이블과 매핑`되었기에 **대부분의 필드들이 테이블의 컬럼과 매칭되는 경우가 많다.**          
...
그렇기 때문에 **절대로 Entity를 요청이나 응답값을 전달하는 클래스로 사용해서는 안 된다.**             
왜냐하면 언제 어디서든지 값이 변동 될 수 있고 이로 인해 DB 테이블의 값을 변경할 수 있기 때문이다.(더티 체크)     
     
**Entity의 특징은 아래와 같다.**           
1. Entity를 식별할 수 있는 ID(식별자)가 존재해야한다.                 
2. ID(식별자)를 통해 Entity를 지속해서 추적할 수 있다.(찾을 수 있다.)                    
3. ID(식별자)를 변경하지 않는 한, 내부 값을 변경해도 식별자를 통해 가져올 수 있다.                
